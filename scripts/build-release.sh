#!/bin/bash
# Build, sign, package, notarize, and staple AgentMonitor for release
...
VERSION="1.0.0"
BUNDLE_ID="${BUNDLE_ID:-com.agentmonitor.app}"
...
cd "$PROJECT_ROOT/app"
xcodebuild -project AgentMonitor.xcodeproj -scheme AgentMonitor -configuration Release -derivedDataPath ./build clean build 2>&1 | grep -E "(error:|warning:|BUILD|Compiling)" || true

APP_PATH="$PROJECT_ROOT/app/build/Build/Products/Release/${APP_NAME}.app"
if [ ! -d "$APP_PATH" ]; then
...
# Step 1b: Copy app icon into bundle (ensure icon is included)
echo ""
echo "🎨 Adding app icon to bundle..."
mkdir -p "$APP_PATH/Contents/Resources"
ICON_SRC="$PROJECT_ROOT/app/AgentMonitor/AgentMonitor.icns"
if [ -f "$ICON_SRC" ]; then
    ...
    echo "   ⚠️ Icon not found at $ICON_SRC (will use default)"
fi
...
ICON_PATH="$PROJECT_ROOT/app/AgentMonitor/AgentMonitor.icns"
